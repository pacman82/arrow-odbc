...
use log::debug;
use odbc_api::{
    AsyncResultSetMetadata, ColumnDescription, DataType as OdbcDataType, ResultSetMetadata, Sleep,
};
...
pub async fn arrow_schema_from_async<S: Sleep>(
    ...
    let mut fields = Vec::new();
    for index in 0..num_cols {
        let field = arrow_field_from_async(result_set_metadata, index, &sleep).await?;
